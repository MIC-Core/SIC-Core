...
The metadata structure defined in this specification follows a simulation process aligned with the https://modelica.github.io/ssp-ls-traceability/1.0.0/#sec-crediblesimulationprocessframework[*Credible Simulation Process Framework*], as defined in <<CSP>>, as referenced in the SSP Layered Standard Traceability 1.0.0 specification (see <<SSPLST>>):
...
For a number of attributes, for example for <<Simulation Task,Simulation Task>>, the specification allows the use of sub-attributes to describe the attribute in more detail, e.g. by providing both machine-readable and human-readable variants of a name, version information, or URIs.

The use of sub-attributes is per-se optional, however standards implementing this specification can make the use of sub-attributes, and the use of certain sub-attributes mandatory.

The list of allowed sub-attributes is listed in the definition of an attribute.
Since they are shared between attribute definitions, the definition of common sub-attributes is placed in <<common-sub-attributes>>.
